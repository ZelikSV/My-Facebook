...
describe('composer component: ', () => {
    ...
    test('textarea should have value be empty', () => {
    ...
    test('_createPost prop should be invoked once form submit', () =>{
        expect(props._createPost).toHaveBeenCalledTimes(1);
    });
    test('_submitComment and _handleFormSubmit methods', () =>{
        expect(_submitCommentSpy).toHaveBeenCalledTimes(1);
        expect(_handleFormSubmitSpy).toHaveBeenCalledTimes(1);
    });
    test('src of img must be a string', () =>{
        expect(typeof result.prop('avatar')).toBe('string');
    });
    test('currentUserFirstName must be a string', () => {
